...
docker exec master bash -c "export HADOOP_CLASSPATH=\`hadoop classpath\` && \
    ...
    /home/hadoop-user/$FLINK_DIRNAME/bin/flink run-application -t yarn-application -Dtaskmanager.memory.process.size=1500m -jobmanager.memory.process.size=1000m \
    -pyfs /tmp/add_one.py \
